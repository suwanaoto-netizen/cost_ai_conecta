...
3. 担当者が内容を確認・修正して「入力済み」にし、車両コスト管理システム（ロジポケ等）へ **データ連携** する。
4. 連携済みデータから **車両（車番）単位のコストを自動集計**し、コストモニターで可視化する。

### 1.3 コアバリュー
- 転記レス: OCR＋突合で入力を自動化。
- 車両原価の可視化: 1車番＝1レコードで常に最新の積み上げを表示。
- 監査性: 連携済みデータの編集は変更履歴（いつ・誰が・何を・どう変えたか）として記録。
...
| コスト分類 (Cost Category / cat) | 費用の会計区分。`燃料費 / 修繕・維持費 / 通行料 / 保険料 / 調達コスト / 税金`（**マスタデータ＞コスト分類タブ**で追加・削除可）。連携用の内訳コード（subCat）を派生で持つ。 |
...
| ロジポケ (Logipoke) | データ連携先の車両コスト管理システム。API連携（エンドポイント＋APIキー）。モビポケ・外部サービスと並ぶ連携先のひとつ。 |
| データ連携 (Reflect) | 入力済み書類のコストを連携先（ロジポケ＝コストモニター 等）へ確定計上すること。旧称「ロジポケに反映」。 |
| 変更履歴 (Changelog) | 連携済みコストへの編集差分の監査ログ。 |
...
| 入力済み (st-done) | 内容を確認・確定済み。車両コストへデータ連携可能。 | ドット付きチップ |
| 連携済み (st-reflected) | コストへ連携済み。書類一覧では**編集不可（🔒）**。修正はコストモニターから。 | 鍵アイコン付きチップ |
...
type Vehicle = {         // コストモニターの集計レコード（連携済みから動的生成）
  key: string;           // 車番ベースのキー
  target: string;        // 車番
  kind: string;
...
- トースト: `toast(msg, {label, fn})` — アクション付き通知（例: データ連携後に「コストモニターを見る」）。
...
- **ステッパー**（3段）: ①取込・データ化（常にdone）→ ②入力済みにする → ③データ連携。現ステータスで `current/done/todo` を切替。
- アクションボタン:
  - 「入力済み」（`savePanel`）— 未入力時に有効、連携済みでは非表示。
  - 「データ連携する」（`reflectPanel`）— **入力済みのときのみ活性**（未入力時はヒント表示）。連携済みは「連携済み（日時）」バッジ。
...
**コスト分類 タブ**（コスト構造のマスタ。`master` ストアの `costCats`）:
- 分類ごとに行表示: コスト分類ピル（色は `catStyleOf`）/ **想定書類タイプ**（チップトグル、書類タイプのヒント・コストモニター内訳集計に使用）/ **連携用内訳コード**（`subcatsOf` から派生、参照のみ）。
- 分類の追加（ヘッダの入力＋ボタン）・削除（行末×、削除確認モーダルで連携済みデータへの影響を警告）。即時反映（車両マスタと同じ）。
- 内訳コード（subCat）と自動判定ルールは外部連携の契約キーのため `domain/repairSubcat.ts` が真実源（UIからは変更不可）。
...
6. データ連携: タブ切替（`dataLinkTab`）で **ロジポケ連携 / モビポケ連携 / 外部サービス連携** を設定（`renderDataLinkCard`）。
...
- ドメインデータ: `docs`, `lines`, `vehicleMaster`, `changelog`, `categories(営業所)`, `costCats(コスト分類マスタ：分類名＋想定書類タイプ。master ストア)`
...
| データ連携・接続テスト | ロジポケ: `logipokeUrl`+`logipokeKey`、`POST /integrations/logipoke/test`。モビポケ: `mobipokeUrl`+`mobipokeKey`、`POST /integrations/mobipoke/test`。外部サービス: `externalUrl`+`externalKey`+`externalFormat`、`POST /integrations/external/test` |
| 変更履歴 | `GET /changelog` |
