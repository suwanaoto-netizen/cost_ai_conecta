...
![図1. 書類一覧 — 一覧／ステータスフィルタ／検索／一括処理。左サイドナビ・上部にパンくずと連携アラートのベル。](img/01-documents.png)
...
![図2. 書類詳細パネル（右スライド）— ステッパー／書類情報／明細テーブル（突合バッジ・内訳）と、右に請求書原本プレビュー。](img/02-doc-panel.png)
...
![図3. アップロードモーダル — 新規／PDF分割の2タブ、ファイル選択・営業所・AI-OCRトグル。](img/03-upload.png)
...
![図4. コストモニタータブ — KPI・ピックアップ3リスト・推移グラフ（車両維持コスト推移／営業所別コスト分類推移）。](img/04-monitor.png)
...
![図5. 個別モニタータブ — 車両ごとの集計テーブル（マスタ区分・コスト内訳バー・最終発生／明細数・累計コスト・編集）。](img/05-monitor-individual.png)
...
![図7. 連携アラート — Topbarのベルから開くポップオーバー。増加（赤）／ピックアップ変化（青）を区別。](img/09-alerts.png)
...
![図6. 車両カルテ — 累計コスト／明細数／最終発生、コスト内訳、発生明細のタイムライン。](img/06-vehicle-karte.png)

### 4.5 マスタデータ (master)

**目的**: 突合・集計・連携の真実源となるマスタデータの管理。**タブ構成（車両マスタ / コスト分類）**。

![図8. マスタデータ（車両マスタ）— 車番・車台番号・営業所・諸元の一覧／編集。CSV出力・自検協突合。](img/07-master.png)
...
![図9. 設定 — 会社情報／AI-OCR・突合しきい値／表示／営業所管理／コスト分類導線／データ連携（タブ）。](img/08-settings.png)
