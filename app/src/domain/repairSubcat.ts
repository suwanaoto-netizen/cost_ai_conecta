...
/** 明細品名 → 内訳コードの推定ルール（上から順に最初の一致を採用）。 */
const RULES: { code: string; kw: RegExp }[] = [
  { code: "shaken", kw: /車検/ },
  { code: "inspection", kw: /点検/ },
  { code: "battery", kw: /バッテリ/ },
  { code: "tire", kw: /タイヤ|スタッドレス|チューブ|ホイール|バルブ/ },
  { code: "oil", kw: /オイル|エレメント|フィルタ|ＬＬＣ|LLC|クーラント|ウォッシャ|グリス|フルード/ },
  { code: "wash", kw: /洗車|ワックス|シャンプー|清掃|ウエス|美装/ },
  { code: "onboard", kw: /電球|ヒューズ|ワイパー|発煙筒|軍手|荷締/ },
  { code: "repair", kw: /修理|交換|板金|塗装|バンパー|パネル|ベルト|ガス補充|整備/ },
];

/** 品名から修繕・維持費の内訳コードを推定する。未該当はドライバー利用消耗品扱い。 */
export function inferRepairSubcat(item: string): string {
  return RULES.find((r) => r.kw.test(item))?.code ?? "consumable";
}
